...
def squat(savefilename):
    for i in range(0, 301):
        print("\t\t\t========\tsquat i:", i,"\t\t========\n")
        im_fname = '/home/jos/tf_lab/Ultralight-SimplePose/generator_dataset/2_res_image/squat/jos'+str(i)+'.jpg'
        ...
        for x in scores[0]:
            if x > 0.5:
                ...
                # variabel for sit 3
                distance_from_central.append(1)
...
def stand(savefilename):
    for i in range(0, 310):
        print("\t\t\t========\tstand i:", i,"\t\t========\n")
        im_fname = '/home/jos/tf_lab/Ultralight-SimplePose/generator_dataset/2_res_image/stand/jos'+str(i)+'.jpg'
        x, img = data.transforms.presets.ssd.load_test(im_fname, short=512) # print('Shape of pre processed image: ', x.shape)
        class_IDs, scores, bounding_boxs = detector(x)
        pose_input, upscale_bbox = detector_to_simple_pose(img, class_IDs, scores, bounding_boxs)
        predicted_heatmap = pose_net(pose_input)
        ...
        for x in scores[0]:
            if x > 0.5:
                ...
                # variabel for stand 2
                # variabel for sit 3
                distance_from_central.append(2)
                with open(savefilename, mode='a') as squat:
                    distance_writer = csv.writer(
                        squat, delimiter=',', quoting=csv.QUOTE_MINIMAL)
                    distance_writer.writerow(distance_from_central)
...
def sit(savefilename):
    for i in range(100, 301):
        print("\t\t\t========\tsit i:", i,"\t\t========\n")
        im_fname = '/home/jos/tf_lab/Ultralight-SimplePose/generator_dataset/2_res_image/sit/jos'+str(i)+'.jpg'
        x, img = data.transforms.presets.ssd.load_test(im_fname, short=512) # print('Shape of pre processed image: ', x.shape)
        class_IDs, scores, bounding_boxs = detector(x)
        pose_input, upscale_bbox = detector_to_simple_pose(img, class_IDs, scores, bounding_boxs)
        predicted_heatmap = pose_net(pose_input)
